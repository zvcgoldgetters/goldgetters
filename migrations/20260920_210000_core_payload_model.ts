...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`club_settings\` (
	  ...
	  \`updated_at\` text,
	  \`created_at\` text,
	  FOREIGN KEY (\`current_team_id\`) REFERENCES \`teams\`(\`id\`) ON UPDATE no action ON DELETE set null,
	  FOREIGN KEY (\`current_season_id\`) REFERENCES \`seasons\`(\`id\`) ON UPDATE no action ON DELETE set null,
	  FOREIGN KEY (\`current_league_id\`) REFERENCES \`leagues\`(\`id\`) ON UPDATE no action ON DELETE set null,
	  FOREIGN KEY (\`default_venue_id\`) REFERENCES \`venues\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `);
  await db.run(
    sql`CREATE INDEX \`club_settings_current_team_idx\` ON \`club_settings\` (\`current_team_id\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`club_settings_current_season_idx\` ON \`club_settings\` (\`current_season_id\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`club_settings_current_league_idx\` ON \`club_settings\` (\`current_league_id\`);`,
  );
  await db.run(
    sql`CREATE INDEX \`club_settings_default_venue_idx\` ON \`club_settings\` (\`default_venue_id\`);`,
  );
